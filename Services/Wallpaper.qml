...
QtObject {
    ...
    function apply(path) {
        ...
        _apply.command = ["sh", "-c",
            ...
            " && kwriteconfig6 --file kscreenlockerrc --group Greeter --group Wallpaper --group org.kde.image --group General --key Image " + _quote(path)
            + "; $HOME/.local/bin/starlite-login-sync --wallpaper " + _quote(path) + " >/dev/null 2>&1 || true"]   // plain path: Plasma accepts it
        _apply.running = true
    }
